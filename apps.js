...
sections.forEach((section, i) => { // to loop over sections 
    const list = document.createElement("li"); //create list and put it in a variable
    const links = document.createElement("a"); //create anchors and put it in a variable
    links.innerText = section.getAttribute("data-nav");
    links.classList.add("menu__link"); //add class to links
    links.setAttribute("id", `menu-${i + 1}`); //add id
    ...
    links.addEventListener('click', (event) => {
        ...
        section.scrollIntoView({
            behavior: "smooth",
            block: "start"
        }); // to scroll smoothly between sections when click on the links 
    })
